...
layout = [
    [sg.Text("キーワードリンクを除去するはてなブログ記事のURLを入力し、OKを押してください")],
    [sg.Input(key="-IN-")],
...
layout = [
    [sg.Text("記事の｢HTML編集｣本文を入力し、OKを押してください")],
        ...
        linked_keywords = sorted(
            list(
                set(
                    ...
                    if "https://d.hatena.ne.jp/keyword/" in str(url.get("href"))
                )
            ),
            key=len,
        )
        ...
        for lk in linked_keywords:
            for with_lk in soup_html.find_all(text=re.compile(".*" + lk + ".*")):
                ...
                if s[0] != "[" or s[-1] != "]":  # Twitter埋め込みなどのコマンドを置換候補から除く
                    with_lk.replace_with(
                        s.replace("[]" + lk + "[]", lk).replace(lk, "[]" + lk + "[]")
                    )  # リンク付き単語を[]で囲む。2重付与しないよう既に囲まれたものを戻してから
